...
export function buildUserPrompt(type, data) {
  const ctx = buildDataContext(data);
  if (type === "synthesis") return ctx + SYNTHESIS_TAIL;
  if (type === "objectives") return ctx + OBJECTIVES_TAIL;
  if (type === "development") return ctx + DEVELOPMENT_TAIL;
